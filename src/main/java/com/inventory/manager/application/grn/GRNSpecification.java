...
public class GRNSpecification {
    ...
    public boolean isSatisfiedBy(CreateGoodReceiveNoteRequestDTO requestDTO, List<Item> items, List<Integer> itemIds,
            Location location) {
        ...
        if (location == null) {
            throw new NotFoundException("Location not found for the ids: " + requestDTO.getLocationId());
        }

        for (GoodReceiveNoteLineRequestDTO line : requestDTO.getGoodReceiveNoteLines()) {
            ...
            if(requestDTO.isReturn() && line.getQuantity() >= 0){
                throw new BadRequestException(CustomExceptionCodes.INVALID_QUANTITY.toString(),
                        "Quantity should be negative on return GRN");
            }
